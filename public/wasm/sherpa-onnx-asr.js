...
function createOnlineRecognizer(Module, myConfig) {
  ...
  // pitfall: 原来是 recognizerConfig = myConfig 直接替换，modelConfig 等字段丢失
  // 导致 OnlineRecognizer 构造时 freeConfig(undefined) 报错
  // 必须用 Object.assign 合并，只覆盖调用方传入的字段
  if (myConfig) {
    Object.assign(recognizerConfig, myConfig);
  }
