const strings: Record<string, Record<string, string>> = {
  zh_CN: {
    ...
    translateZhEn: '中→英',
    translateEnZh: '英→中',
    translateTimingStream: '实时跟句',
    translateTimingFinal: '仅定稿',
    helpTranslateTiming: '实时跟句：识别过程中反复重译中间结果，出译文快但更耗 CPU（每 0.5 秒最多一次）；仅定稿：每句说完才翻译一次，最省资源、译文不闪变。设置从下一次启动会话生效。',
    pickModel: '选择模型',
    ...
    asrModelHint: '当前安装包未内置语音识别模型（约 412MB），点击下方按钮直接下载到本地，之后不会再询问。',
    asrModelDownloadBtn: '一键下载模型',
    asrModelDownloading: '正在下载模型…',
    asrModelDlKeepOpen: '⚠ 下载进行中，请保持此窗口打开（点击窗口外也会中断下载）',
    asrModelDownloadDone: '下载完成，正在导入…',
    asrModelDownloadErr: '下载失败，请检查网络或使用备用方案',
    asrModelAltToggle: '备用方案：手动下载后导入',
    asrModelModelScope: '从 ModelScope 下载模型（推荐，速度快）',
    asrModelGithub: '从 GitHub Releases 下载模型',
    asrModelGitee: '从 Gitee 下载模型（备用，国内 GitHub 不通时使用）',
    ...
    sourceHintFirefox: 'Firefox 无法直接捕获标签页/系统音频（无 tabCapture，getDisplayMedia 忽略 audio），改用「音频输入设备」：开始后会弹出小窗，点按钮授权麦克风并选择输入；装了「扬声器 Monitor 回环」（Linux 系统自带）或「立体声混音」（Windows 声卡）等虚拟输入设备的，整机声音可直接进来。识别期间请保持小窗开启。',
  ...
  en: {
    ...
    translateZhEn: 'ZH→EN',
    translateEnZh: 'EN→ZH',
    translateTimingStream: 'Live',
    translateTimingFinal: 'Final only',
    helpTranslateTiming: 'Live: re-translates the growing text while a sentence is being spoken (at most once per 0.5s) — faster to show, higher CPU. Final only: translates once when each sentence completes — lowest CPU, no flicker. Applies from the next session.',
    ...
    asrModelHint: 'This package does not include the speech recognition model (~412MB). Click below to download it directly — you will not be asked again.',
    asrModelDownloadBtn: 'Download Model',
    asrModelDownloading: 'Downloading model…',
    asrModelDlKeepOpen: '⚠ Download in progress — keep this window open (clicking outside will cancel it)',
    asrModelDownloadDone: 'Download complete, importing…',
    asrModelDownloadErr: 'Download failed. Check your network or use the backup options below.',
    asrModelAltToggle: 'Backup: manual download & import',
    asrModelModelScope: 'Download model from ModelScope (recommended, fastest)',
    asrModelGithub: 'Download model from GitHub Releases',
    asrModelGitee: 'Download model from Gitee (backup for China)',
    ...
    sourceHintFirefox: 'Firefox cannot capture tab/system audio directly (no tabCapture; getDisplayMedia ignores audio), so it uses an audio input instead: a small window appears after Start — click the button to grant microphone access and pick an input. With a "Monitor" loopback device (native on Linux, or "Stereo Mix" on some Windows cards) the whole system sound comes in with zero installs. Keep the window open while running.',
